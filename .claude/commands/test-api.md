...
1. **Find the route** — locate the handler in `apps/api-ts/src/routes/` and understand what it expects (method, path, body schema, auth requirement)
...
If the API isn't running, suggest starting it with `cd apps/api-ts && npm run dev`.
