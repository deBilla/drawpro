...
2. **Dependencies** — check if `node_modules` exist at root and in each app (apps/api-ts, apps/frontend, apps/collab). If missing, suggest `npm install` from root.
...
4. **Database** — check if DATABASE_URL is configured in apps/api-ts/.env. Run `npx prisma migrate status` in apps/api-ts to check migration state.
5. **Redis** — check if REDIS_URL is set. Try `redis-cli ping` to see if Redis is reachable.
6. **MinIO** — check if MINIO_* vars are set in apps/api-ts/.env.
