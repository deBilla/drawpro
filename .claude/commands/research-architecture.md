...
3. **Authentication flow** — JWT access/refresh token lifecycle, read `apps/api-ts/src/routes/auth.ts` and `apps/frontend/src/lib/api.ts`
4. **E2EE flow** — how encryption works end-to-end, read `apps/frontend/src/lib/crypto.ts`, `apps/frontend/src/components/PasscodeSetup.tsx`, and the sheet encryption in `apps/api-ts/src/routes/sheets.ts`
...
6. **Data model** — read `apps/api-ts/prisma/schema.prisma` and explain entity relationships
